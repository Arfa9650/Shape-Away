...
public class Hand : IntEventInvoker
{
    [SerializeField]
    GameObject handText;
    ...
    void Update()
    {
        GameObject currentObject = GameObject.FindGameObjectWithTag(shapeName);

        // Find all GameObjects with the specified tag
        GameObject[] shapes = GameObject.FindGameObjectsWithTag(shapeName + "Trigger");
    ...
    void UpdateShape(int num)
    {
        ...
        translate = true;
        if (shapeName != null)
            shapeName = ((ShapeName)RandSpawner.currentShape).ToString();
        Debug.Log(shapeName);
    }
